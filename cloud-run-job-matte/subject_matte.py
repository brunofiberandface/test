...
# 2026-05-11: reverted to u2net after isnet-general-use OOM'd at 4 Gi
# (and even at 8 Gi the inference at 4K is borderline). Root-cause analysis:
# the "background paint too aggressive" complaint from Bruno was caused by
# `alpha_matting=True` with `erode_size=10` — that erosion was eating into
# hair/body edges, NOT the model choice. Dropping alpha_matting (below)
# gives clean u2net edges within 2 Gi memory.
MODEL_NAME = "u2net"
...
def get_session():
    global _SESSION
    if _SESSION is None:
        log(f"loading {MODEL_NAME} session (first call downloads ~170MB)")
        t0 = time.time()
        _SESSION = new_session(MODEL_NAME)
        log(f"{MODEL_NAME} session loaded in {time.time()-t0:.1f}s")
    return _SESSION
...
def matte_subject(src_path: str) -> tuple[Image.Image, Image.Image]:
    ...
    with open(src_path, "rb") as f:
        raw = f.read()
    # 2026-05-11 (final, after iteration): u2net + alpha_matting=True + erode=5.
    # - u2net (not isnet) — small model, fits 2 Gi. isnet OOMs even at 4 Gi.
    # - alpha_matting=True — needed for sharp clean edges. Without it the
    #   silhouette is too soft / translucent (e-commerce quality fail).
    # - erode_size=5 (was 10) — Bruno's "aggressive cutout" complaint came
    #   from erode=10 eating into hair / body edges. 5 preserves more of
    #   the silhouette while still letting alpha matting refine cleanly.
    out_bytes = remove(
        raw,
        session=get_session(),
        alpha_matting=True,
