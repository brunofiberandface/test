...
interface Model {
  ...
  // 2026-05-16: high-res model assets at 1:1 4K square. Generated by
  // scripts/generate-model-assets.ts. Used as canonical reference set
  // shown in the model modal + as anchors for the native M01/M02 pipeline.
...
export default function ModelsPage() {
  ...
  const [assetLightbox, setAssetLightbox] = useState<string | null>(null);  // URL of 4K asset to view full-res
  ...
  // Close modal on Escape (lightbox closes first if open)
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (e.key !== 'Escape') return;
      if (assetLightbox) { setAssetLightbox(null); return; }
      if (selectedModel) closeModal();
    };
    window.addEventListener('keydown', handleKey);
    return () => window.removeEventListener('keydown', handleKey);
  }, [selectedModel, assetLightbox]);
  ...
  return (
    <Shell user={user}>
            ...
            {modalLoading ? (
              ...
              <div className="flex gap-8 p-8">
                ...
                <div className="flex-shrink-0">
                  ...
                  {/* ── 4K Model Asset Library (2026-05-16) ────────────────────
                      4 high-resolution model reference images at 1:1 square 4K:
                      full body F+B + legs-only F+B. Used as canonical reference
                      set and as anchors for the native M01/M02 pipeline.
                      Generated via scripts/generate-model-assets.ts. */}
                  {(selectedModel.assets4K_fullBodyFront ||
                    selectedModel.assets4K_fullBodyBack ||
                    selectedModel.assets4K_legsFront ||
                    selectedModel.assets4K_legsBack) && (
                    <div className="mt-6">
                      <div className="flex items-baseline justify-between mb-2">
                        <p className="text-[10px] font-medium text-neutral-400 uppercase tracking-wider">
                          4K Model Assets — click to view full resolution
                        </p>
                        {selectedModel.assets4K_updatedAt && (
                          <p className="text-[10px] text-neutral-400">
                            {new Date(selectedModel.assets4K_updatedAt).toLocaleDateString()}
                          </p>
                        )}
                      </div>
                      <div className="grid grid-cols-4 gap-2">
                        {([
                          ['Full body — front', selectedModel.assets4K_fullBodyFront],
                          ['Full body — back', selectedModel.assets4K_fullBodyBack],
                          ['Legs — front (M01)', selectedModel.assets4K_legsFront],
                          ['Legs — back (M02)', selectedModel.assets4K_legsBack],
                        ] as Array<[string, string | undefined]>).map(([label, url], i) => (
                              ...
                              {url ? (
                                <>
                                  ...
                                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/15 transition-colors flex items-center justify-center">
                                    <div className="opacity-0 group-hover:opacity-100 bg-black/70 text-white text-[10px] px-2 py-1 transition-opacity">
                                      Click for 4K
                                    </div>
                                  </div>
                                </>
                              ) : (
                                <div className="w-full h-full flex items-center justify-center text-neutral-300 text-[10px] text-center px-2">
                                  Not generated
                                </div>
                              )}
                            </div>
                            <p className="text-[10px] text-neutral-500 text-center truncate">{label}</p>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {/* 4K asset lightbox — opens when any of the 4 model-asset thumbnails are clicked. */}
      {assetLightbox && (
        <div
          className="fixed inset-0 z-[60] bg-black/90 flex items-center justify-center cursor-zoom-out p-4"
        ...
        >
          ...
          <a
            ...
            onClick={(e) => e.stopPropagation()}
          >
            Open full 4K in new tab
          </a>
          <button
            onClick={() => setAssetLightbox(null)}
            className="absolute top-4 left-4 bg-white/15 hover:bg-white/30 text-white text-xs font-medium px-3 py-1.5 backdrop-blur-sm transition-colors"
          >
            Close (Esc)
          </button>
        </div>
