/**
 * M01 v3 prompt — drop the explicit angle names ("front 45° left", etc).
 *
 * v2 used "viewed from 3 front angles: straight front, front 45° left,
 * front 45° right" but the left/right convention is ambiguous (camera-
 * perspective vs model-perspective). Per Bruno + LEARNING #101: simpler
 * is usually better with Seedream. The visual evidence carries the angle
 * info — Seedream can triangulate without needing the labels parsed.
 *
 * v3: just "from slightly different angles" + flat product photo.
 * 5 refs unchanged. 3 runs.
 */
...
async function main() {
  console.log(`Prompt length: ${M01_V3_PROMPT.length} chars`);
  for (let i = 1; i <= 3; i++) {
    try {
      await runOne(i);
    } catch (e) {
