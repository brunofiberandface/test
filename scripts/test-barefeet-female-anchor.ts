...
const OUT_DIR = path.join(projectRoot, 'test_outputs/sole-mode/barefeet-female');
fs.mkdirSync(OUT_DIR, { recursive: true });
// "with shoes" → "in the photo" — anchor singular subject to the matrix base
// image without conjuring shoes.
const PROMPT = `Apply the fitmodel trousers onto the AI model in the photo. so merge the two images, but the trousers should be fully respected in form and fit. all details need to be preserved, IMPORTANT is the trousers run naturally down respecting the fitmodel lenght of the trousers. the model needs to stand equally on 2 feet, 50% on each foot, keeping the same position as the underwear model.`;
async function main() {
  for (const i of [1, 2, 3]) {
    console.log(`\n[run${i}] Seedream ("in the photo" anchor)…`);
    const t0 = Date.now();
