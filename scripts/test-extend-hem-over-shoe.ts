/**
 * v2 of the shoes-under-hem approach. Previous prompt tried to preserve the
 * existing hem position; this prompt explicitly tells Gemini to EXTEND the
 * pant legs DOWNWARD to cover the shoes — the failure mode of v1 was Gemini
 * either lifting the hem (run 1) or leaving the hem above the shoe (run 3).
 *
 * Same 3 refs: barefoot base, AXEL ARIGATO shoe, cream-pant-over-sneaker
 * layering example.
 *
 * 3 runs.
 */
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    try {
      const result = await generateImage({
        prompt: PROMPT,
        referenceImages: [
          { buffer: baseBuf, mimeType: 'image/png', label: 'IMAGE 1 — base barefoot CONTOR render. Preserve model, jeans construction, scene. Extend the pant legs downward to cover shoes.' },
          { buffer: shoeBuf, mimeType: 'image/png', label: 'IMAGE 2 — AXEL ARIGATO white low-top sneaker back view. Add to model feet under extended denim.' },
          { buffer: layeringBuf, mimeType: 'image/png', label: 'IMAGE 3 — layering reference (pant cascading OVER sneaker). Spatial-relationship guidance only.' },
        ],
        aspectRatio: '1:1',
        imageSize: '4K',
