/**
 * v6 — keep v5's 3 back refs (matrix + back + back45L + back45R) but
 * strengthen the framing line so Seedream stops extending upward beyond
 * the matrix base's waist-down crop.
 *
 * v5 problem: with 4 refs, run 3 had full bare back from neck down —
 * the "no head, no upper body" parenthetical was too weak.
 *
 * v6 fix: replace the parenthetical with a positive-spec line that
 * matches IMAGE 1's exact crop, with ONE short negative ("Frame above
 * the waistband is not rendered") — avoid the v3-style multi-negative
 * list that destabilizes.
 *
 * 3 runs.
 */
...
const OUT_DIR = path.join(projectRoot, 'test_outputs/sole-mode/m02-lean-prompt-audit-v6');
