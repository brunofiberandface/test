/**
 * Upload a matrix-paint vault prompt file (M01 or M02) as the next active
 * revision. Deactivates the prior active rev in the same batch.
 *
 * Usage:
 *   npx tsx scripts/upload-matrix-vault.ts <M01|M02> <path/to/file.md>
 */
...
const SHOT_TYPE = process.argv[2];
const FILE_REL = process.argv[3];
if (!SHOT_TYPE || !FILE_REL) {
  console.error('Usage: npx tsx scripts/upload-matrix-vault.ts <M01|M02> <path/to/file.md>');
  process.exit(1);
}
if (SHOT_TYPE !== 'M01' && SHOT_TYPE !== 'M02') {
  console.error(`Unsupported shotType ${SHOT_TYPE}. Only M01 / M02 supported here.`);
  process.exit(1);
}
const FILE_PATH = path.isAbsolute(FILE_REL) ? FILE_REL : path.join(projectRoot, FILE_REL);
...
async function main() {
  ...
  batch.set(newDoc, {
    ...
    uploadedBy: 'claude-bytedance-realign',
  });

  await batch.commit();
  console.log(`✓ Uploaded ${SHOT_TYPE} rev ${newRev} (id=${newDoc.id}), deactivated ${deactivated} prior active rev(s)`);
  process.exit(0);
}
