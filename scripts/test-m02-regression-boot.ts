/**
 * Regression check: M02 lean vs lean+layer-order on CONTOR × brown ankle boot.
 *
 * The layer-order block has a separate "boots" branch saying the pant falls
 * OUTSIDE the boot shaft. If that branch is OK for the boot case, no regression.
 * If lean+layer-order somehow regresses boots, that's a blocker for promotion.
 *
 * 2 variants × 3 runs = 6 renders.
 */
...
const BOOT_ID = 'C0QleJPtkd58NRiHhPES'; // Ankle boot brown suede
const F_MODEL_ID = 'F3';
const RUNS = 3;

const OUT_DIR = path.join(projectRoot, 'test_outputs/sole-mode/m02-regression-boot');
...
async function main() {
  ...
  const matrixDoc = await db.collection('qaShoeMatrix').doc(`${BOOT_ID}_${F_MODEL_ID}`).get();
  const baseUrl = (matrixDoc.data() as any).images?.legsBack;
  console.log(`Boot matrix base: ${baseUrl}`);
    ...
    for (let i = 1; i <= RUNS; i++) {
      ...
      try {
        const result = await generateSeedreamImage({
          prompt,
          referenceImages: [
            { url: baseUrl, label: 'IMAGE 1: matrix base (model in brown ankle boots)' },
        ...
        fs.writeFileSync(path.join(OUT_DIR, `${variant}_run${i}.png`), result.imageData);
