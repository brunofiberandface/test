...
const OUT_DIR = path.join(projectRoot, 'test_outputs/sole-mode/barefeet-noshoes');
fs.mkdirSync(OUT_DIR, { recursive: true });
// "with shoes" → removed; "trousers" → trousers naturally cascade with no shoe anchor
const NO_SHOES_PROMPT = `Apply the fitmodel trousers onto the AI model. so merge the two images, but the trousers should be fully respected in form and fit. all details need to be preserved, IMPORTANT is the trousers run naturally down respecting the fitmodel lenght of the trousers. the model needs to stand equally on 2 feet, 50% on each foot, keeping the same position as the underwear model.`;
async function main() {
  for (const i of [1, 2, 3]) {
    console.log(`\n[run${i}] Seedream (no "shoes" in prompt)…`);
    const t0 = Date.now();
    const result = await generateSeedreamImage({
      prompt: NO_SHOES_PROMPT,
