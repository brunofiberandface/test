/**
 * Upload an M06 vault prompt file as the next active revision (any pipeline).
 * Same logic as upload-matrix-vault.ts but hardcoded for M06.
 *
 * Usage: npx tsx scripts/upload-m06-vault.ts <path/to/file.md>
 */
...
const FILE_REL = process.argv[2];
if (!FILE_REL) { console.error('Usage: npx tsx scripts/upload-m06-vault.ts <path/to/file.md>'); process.exit(1); }
...
async function main() {
  ...
  if (!generationPrompt) { console.error('Could not extract Step 2 Prompt section'); process.exit(1); }
