/**
 * v8 — same as v7 (5 refs with matrix anchor at both ends), add one
 * positive arms line to fix the missing-arms regression.
 *
 * Bug found in v7 (and likely v4/v5/v6 too): the Tier-1 base has arms
 * hanging at the sides, but the Judee fit-model back refs don't show
 * arms (model cropped to torso + jeans). Seedream inherited the
 * armless composition from the fit-model refs.
 *
 * Fix: explicit arms line — "Arms hang naturally at both sides of the
 * body, visible from the shoulders down past the hips, as in IMAGE 1."
 *
 * 3 runs.
 */
...
const OUT_DIR = path.join(projectRoot, 'test_outputs/sole-mode/m02-lean-prompt-audit-v8');
fs.mkdirSync(OUT_DIR, { recursive: true });

const LEAN_PROMPT = `Merge IMAGE 1 (barefoot AI model in black placeholder underwear, waist-down) with IMAGES 2-4 (fitmodel wearing the target trousers, viewed from 3 back angles: straight back, back 45° left, back 45° right). Output = IMAGE 1 with the placeholder replaced by the trousers from IMAGES 2-4. IMAGE 5 is IMAGE 1 repeated as the closing anchor — its framing, model identity, arms, and bare-feet pose are the final ground truth for the output. Preserve the trousers' wash, color, fit, length, drape, hem behavior, width progression, and back-pocket construction exactly as in IMAGES 2-4 — use the 3 angles to triangulate the true garment silhouette. Preserve IMAGE 1's exact crop: the top edge of the output frame is at the waistband level (exactly matching IMAGE 1's top edge), model centered horizontally, bottom edge just below the feet. Frame above the waistband is not rendered. Preserve model identity, anatomical body orientation (entire back of body facing the camera as in IMAGE 1), arms hanging naturally at both sides of the body (visible from the shoulders down past the hips, as in IMAGE 1), and bare feet. Render exactly ONE model.`;
