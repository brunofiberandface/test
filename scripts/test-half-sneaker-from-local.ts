/**
 * Skip the Gemini half-sneaker conversion (already saved locally as
 * halfsneaker_base.png from a prior run that crashed on GCS upload).
 * Upload existing file → run 3 Seedream CONTOR paints against it.
 */
...
async function main() {
  console.log('Uploading existing half-sneaker base to GCS...');
  ...
  const tempId = `halfsneaker-${Date.now()}`;
  const gcsKey = `experiments/sole-mode/${tempId}/base.png`;
  const baseBuf = fs.readFileSync(LOCAL_BASE);
  await bucket.file(gcsKey).save(baseBuf, { metadata: { contentType: 'image/png' } });
  // Make publicly readable (Seedream needs it)
  await bucket.file(gcsKey).makePublic().catch(() => { /* may already be public */ });
  ...
  for (const i of [1, 2, 3]) {
    ...
    console.log(`[run${i}] DONE in ${dt}s — ${(result.imageData.length / 1024).toFixed(0)}KB → ${out}`);
  }
