/**
 * v2: Push Gemini to honor the low-profile cut from IMAGE 2.
 *
 * v1 problem: Gemini rendered fuller shoes ignoring the sole+1cm lip cut.
 * v2 strategy:
 *  - Avoid generic shoe nouns ("sneaker", "boot") that pull Gemini's training-set fuller shape.
 *  - Describe the asset as "backless mule-cut footwear" with explicit no-extension constraint.
 *  - Repeat the cut constraint in 3 places (intro, placement instruction, occlusion block).
 */
...
const OUT_DIR = path.join(projectRoot, 'test_outputs/sole-mode/low-profile-composite-v2');
...
async function main() {
  ...
  for (const shoe of SHOES) {
    console.log(`\n=== ${shoe.id} (${shoe.material}) ===`);
    const assetBuf = fs.readFileSync(shoe.asset);

    const prompt = `IMAGE 1 is a back-view photograph of a model in wide-leg jeans, currently barefoot. IMAGE 2 shows a pair of LOW-PROFILE BACKLESS FOOTWEAR in ${shoe.material} on a neutral studio floor — heels facing camera, toes pointing away. The footwear consists ONLY of the sole plus a roughly 1cm lip of material above the sole. There is no full upper, no tongue, no laces, no ankle collar, no heel cup — those parts have been cut away.
...
Match material and colour from IMAGE 2 exactly: ${shoe.material}, with the sole construction visible in IMAGE 2. Do NOT invent branding or text.
...
PRESERVE everything else in IMAGE 1 exactly: model identity, body, pose, arms, upper torso, the pant silhouette and construction (panel seams, anatomical outseams, knee panels, washes, pockets, waistband), backdrop, lighting, framing.`;
    ...
    try {
      const result = await generateImage({
        prompt,
        referenceImages: [
          { buffer: srcBuf, mimeType: 'image/png', label: 'IMAGE 1 — bare-feet target render with wide-leg jeans. Preserve everything except replace bare feet with low-profile footwear from IMAGE 2.' },
          { buffer: assetBuf, mimeType: 'image/png', label: `IMAGE 2 — low-profile backless ${shoe.material} footwear (sole + ~1cm lip only, no upper). Place exactly this pair, keep the low-profile cut.` },
        ],
        aspectRatio: '1:1',
        imageSize: '4K',
