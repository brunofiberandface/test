/**
 * Step 3 — M01 re-render with FRAMING & COMPOSITION block (step-2 head-crop fix).
 *
 * Changes from step 2 (M01 only — M02 was clean, untouched):
 *   1. Replaced "COMPOSITION — MODEL CENTERED" block with "FRAMING & COMPOSITION — MATCH IMAGE 1"
 *      — drops the "same scale" wording (over-zoom trigger), adds explicit
 *      spatial anchors at top (head + margin above) and bottom (feet + margin below).
 *   2. Removed the redundant "Horizontal position … centered" line from the
 *      NARROW EDIT block (now covered by FRAMING).
 *   3. Removed the "DO NOT shift the model left or right…" line from FORBIDDEN
 *      (anti-X risk per LEARNING #55; positive coverage now in FRAMING).
 *
 * Output: step3_M01_pass1.png
 */
...
═══ FRAMING & COMPOSITION — MATCH IMAGE 1 ═══
The output matches IMAGE 1's framing exactly:
- The TOP OF THE MODEL'S HEAD is visible near the top of the frame, with a small margin of backdrop above
- BOTH FEET are visible standing on the floor near the bottom of the frame, with a small margin of floor below
- The model's body is centered HORIZONTALLY — equal empty backdrop on the left and right sides
- The model occupies the same vertical extent in the frame as in IMAGE 1 — head-to-toe within the square
...
async function main() {
  console.log('Step 3 — M01 re-render with FRAMING block fix (head + feet anchored)\n');
  ...
  const out = path.join(OUT_DIR, 'step3_M01_pass1.png');
