/**
 * v5 — same lean+body-orientation prompt as v4, but Seedream receives all
 * THREE back fit-model angles (back + back45Left + back45Right) instead of
 * just the canonical back.
 *
 * Hypothesis: more fit-data = better silhouette + fit fidelity. Seedream
 * can triangulate the garment's actual back width / drape from multiple
 * angles.
 *
 * Note: back45Left + back45Right are still 1333×2000 (not yet 4K-backfilled).
 * The canonical back (fitmodel_05) is 4K. Mixed-resolution refs.
 *
 * 3 runs.
 */
...
const OUT_DIR = path.join(projectRoot, 'test_outputs/sole-mode/m02-lean-prompt-audit-v5-3refs');
fs.mkdirSync(OUT_DIR, { recursive: true });

// Same v4 prompt — just describes "fitmodel trousers" generically; Seedream
// will see the same garment across all 3 fit-model refs (different angles).
const LEAN_PROMPT = `Merge IMAGE 1 (barefoot AI model in black placeholder underwear, waist-down) with IMAGES 2-4 (fitmodel wearing the target trousers, viewed from 3 back angles: straight back, back 45° left, back 45° right). Output = IMAGE 1 with the placeholder replaced by the trousers from IMAGES 2-4. Preserve the trousers' wash, color, fit, length, drape, hem behavior, width progression, and back-pocket construction exactly as in IMAGES 2-4 — use the 3 angles to triangulate the true garment silhouette. Preserve IMAGE 1's framing (1:1 waist-down, centered, no head, no upper body), model identity, anatomical body orientation (entire back of body facing the camera as in IMAGE 1), and bare feet. Render exactly ONE model.`;
...
async function main() {
  ...
  console.log(`Refs: tier-1 F9 legsBack (4K) + Judee back (4K) + Judee back45Left (2K) + Judee back45Right (2K)`);
  for (let i = 1; i <= 3; i++) {
    try {
      await runOne(i);
    } catch (e) {
